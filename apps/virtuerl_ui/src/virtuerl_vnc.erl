...
start(Parent, DomainId, Node) ->
    % TODO: we currently don't link as it's a bit flaky atm
    wx_object:start(?MODULE, [Parent, DomainId, Node], []).
...
init([Parent, DomainId, Node]) ->
    ...
    {Panel, #state{socket = VncProxy, parent = Parent, panel = Panel}}.


handle_info(setup_gl, #state{panel = Panel, socket = VncProxy} = State) ->
    ...
    {noreply, State#state{tex_id = TexId, vnc_conf = {VncWidth, VncHeight}}};
